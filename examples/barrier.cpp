...
void* threadFunction(void* arg) {
    // Unpack tuple manually for C++11 compatibility
    auto* tuplePtr = reinterpret_cast<std::tuple<DataRaceDetector*, SharedVariable*, Lock*, pthread_barrier_t*, Thread*, bool>*>(arg);
    DataRaceDetector* drd = std::get<0>(*tuplePtr);
    SharedVariable* var1 = std::get<1>(*tuplePtr);
    Lock* lock1 = std::get<2>(*tuplePtr);
    pthread_barrier_t* barrier = std::get<3>(*tuplePtr);
    Thread* thread = std::get<4>(*tuplePtr);
    bool mixedAccess = std::get<5>(*tuplePtr);
